...
pub trait TokenLog: Send + Sync {
    ...
    ///   as configured by [`ServerConfig::validation_token_lifetime`].
    ///
    /// Should record that the token was used, and return a token reuse error if the token was
    /// already used previously. False negatives and false positives are both permissible.
    fn check_and_insert(
        &self,
        rand: u128,
        issued: SystemTime,
...
/// even at extreme amounts of usage, while allowing us to store twice as many in the hash set
/// variant.
///
...
fn optimal_k_num(num_bytes: usize, expected_hits: u64) -> u32 {
    assert!(
        expected_hits > 0,
        "BloomTokenReusePreventer expected hits too low"
    );
    let num_bits = (num_bytes as u64)
        .checked_mul(8)
        .expect("BloomTokenReusePreventer num bytes too high");
